...
```{r, eval=FALSE}
odir <- "audio" # Change this to a location on your computer
...
Pitch/frequency is a really useful piece of data for identifying "different sounds." For human speech, it is useful for voice recognition. Finally, if you want to save a sound file, you camn use the `writeWave()` function.
...
Let's grab the photos of the Methodology Department's senior leadership team from the page <https://www.lse.ac.uk/Methodology/People> and then look at how they're represented as data in R.

First, we do some basic webscraping like you've already learned. Start by loading the relevant packages, creating a directory to store the data and reading the relevant HTML into R.
...
odir <- "images" # Change this to a location on your computer
